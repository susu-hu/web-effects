	...
	var from = function(selectorOrElement, selectedPlugins) {
			var parent = selectorOrElement.blur ? selectorOrElement : document.querySelector(selectorOrElement),
				...
				deactivate = function(slide, index) {
					...
					slide != activeSlide && ['inactive', offsetClass, offsetClass + '-' + Math.abs(offset)].map(
						addClass.bind(0, slide));
				},
				...
				step = function(offset, customData) {
					...
					fire(offset > 0 ? 'next' : 'prev', createEventData(activeSlide, customData)) && activate(
						slideIndex, customData);
				},
